...
export const setVerificationStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await audit(context.userId, `seller.verification.${data.status}`, data.sellerId, {
      reason: data.reason,
    });

    const noticeMap = {
      approved: {
        t: "✅ Your store has been approved",
        m: "Welcome! You can now create products and start selling.",
        s: "critical" as const,
      },
      rejected: {
        t: "Verification rejected",
        m: data.reason || "Your store was not approved. Please review and resubmit.",
        s: "critical" as const,
      },
      suspended: {
        t: "Store suspended",
        m: data.reason || "Your store has been suspended by administration.",
        s: "critical" as const,
      },
...
export const bulkSetVerification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const id of data.sellerIds) {
      await audit(context.userId, `seller.verification.bulk_${data.status}`, id, {
        reason: data.reason,
      });
    }
...
export const adminListSellersForReview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabaseAdmin
      .from("sellers")
      .select(
        "id, business_name, name, slug, category, city, city_id, whatsapp_number, profile_photo_url, cover_photo_url, bio, verification_status, verification_documents, rejection_reason, created_at",
      )
