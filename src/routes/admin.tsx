 ...
 *   1. Overview stats
 *   2. Seller management — verify, block, delete, manual badge grant/revoke
...
function AdminPage() {
  ...
  const loadAll = async () => {
    const [
      ...
      supabase.from("sellers").select("id, business_name, slug, category, city, is_verified, is_blocked").order("created_at", { ascending: false }),
      supabase.from("products").select("id", { count: "exact", head: true }),
      supabase.from("whatsapp_clicks").select("id", { count: "exact", head: true }),
      supabase.from("categories").select("*").order("sort_order"),
      supabase.from("products").select("id, name, price, image_url, is_featured, featured_order, status, sellers(business_name, city)").order("is_featured", { ascending: false }).order("featured_order").limit(100),
  ...
  const deleteSeller = async (id: string, name: string) => {
    ...
    // Products cascade on seller delete (ON DELETE CASCADE in schema)
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          {(["sellers","categories","products","vouches","homepage"] as const).map((t) => (
            <button key={t} onClick={() => setActiveTab(t)} className={tabCls(t)}>
              {t === "sellers" ? "Sellers" : t === "categories" ? "Categories" : t === "products" ? "Products" : t === "vouches" ? "Vouches" : "Homepage"}
            </button>
          ))}
        </div>
        ...
        {activeTab === "sellers" && (
          <section className="mt-6">
            ...
            <div className="space-y-2">
              {sellers.map((s) => (
                <div key={s.id} className={`rounded-xl border bg-card p-3 shadow-warm ${s.is_blocked ? "border-destructive/30 opacity-60" : ""}`}>
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {s.is_blocked && <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-[10px] font-medium text-destructive">BLOCKED</span>}
                      </div>
                    ...
                    <div className="flex shrink-0 flex-wrap items-center gap-1.5 text-xs">
                      {/* Gold badge toggle */}
                      <button
                        onClick={() => toggleVerify(s.id, s.is_verified)}
                      ...
                      {/* Delete */}
                      <button
                        onClick={() => deleteSeller(s.id, s.business_name)}
      ...
      {/* ── Category editor dialog (with ImageUploader) ── */}
