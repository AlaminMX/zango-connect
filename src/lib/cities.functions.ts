  ...
  .handler(async ({ data }) => {
    ...
    const [{ data: sellers }, { data: products }, { data: categories }] = await Promise.all([
      publicSupabase
        .from("sellers")
        .select(
          "id, business_name, slug, category, city, profile_photo_url, cover_photo_url, is_verified, rating, bio",
        )
        .eq("city_id", city.id)
        .eq("status", "active")
      ...
      publicSupabase
        .from("products")
        .select(
          "id, name, price, image_url, stock_status, is_featured, featured_order, sellers!inner(id, business_name, slug, city_id, status, verification_status)",
        )
...
export const adminUpsertCity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: stateId, error: stErr } = await (supabaseAdmin as any).rpc("ensure_state", {
      _name: data.state,
    });
...
export const adminDeleteCity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { count } = await supabaseAdmin
      ...
      .eq("city_id", data.id);
    if ((count ?? 0) > 0)
      throw new Error(`Cannot delete — ${count} sellers reference this city. Disable it instead.`);
