...
function ProductCardContent(p: ProductCardProps) {
  ...
  useEffect(() => {
    setLocalStatus(p.status ?? "active");
  }, [p.status]);
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-surface-warm">
        ...
        {soldOut && (
          <span className="absolute left-2 top-2 rounded-full bg-espresso/90 px-2 py-0.5 text-[10px] font-medium text-background">
            Out of stock
          </span>
        )}
        {low && !soldOut && (
          <span className="absolute left-2 top-2 rounded-full bg-sage px-2 py-0.5 text-[10px] font-medium text-white">
            Selling fast
          </span>
        )}
        <button
      ...
      <div className="flex flex-1 flex-col px-1 pb-1 pt-3">
        ...
        {/* Vendor name independent on its own line with verification badge */}
        {p.seller_name && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
            {p.seller_slug ? (
              <Link
                to="/store/$slug"
                params={{ slug: p.seller_slug }}
            ...
            {p.seller_is_verified && <VerifiedBadge className="h-3.5 w-3.5 shrink-0" />}
          </div>
        )}

        {/* State/City independent on its own line */}
        {p.seller_city && (
          <div className="mt-1">
