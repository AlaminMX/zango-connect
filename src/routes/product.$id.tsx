  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("products")
        .select(
          "id, name, price, description, image_url, image_urls, stock_status, status, seller_id, sellers!inner(id, slug, business_name, city, category, whatsapp_number, profile_photo_url, is_verified, is_blocked, verification_status)",
        )
        .eq("id", id)
          ...
          <div>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              {soldOut ? (
                <Button
                  disabled
                  className="min-h-[48px] flex-1 rounded-full bg-muted text-muted-foreground"
                >
                  Currently out of stock
                </Button>
              ) : (
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("products")
        .select(
          "id, name, price, image_url, stock_status, seller_id, sellers!inner(business_name, city, slug, whatsapp_number, is_blocked, verification_status, is_verified)",
        )
        ...
        .abortSignal(AbortSignal.timeout(8000));
      return (data ?? []).filter(
        (p: any) => !p.sellers?.is_blocked && p.sellers?.verification_status === "approved",
      );
    },
  });
