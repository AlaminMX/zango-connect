...
import {
  downloadProductShareCard,
  nativeShareProductCard,
  canNativeShareImage,
  PRODUCT_SHARE_CARD_WIDTH,
} from "@/lib/product-share-card/export";
import type {
  ProductShareCardProduct,
  ProductShareCardVendor,
} from "@/lib/product-share-card/types";
...
export function ProductShareDialog({ product, vendor }: Props) {
  ...
  const filename = `zango-${(product.name || "product")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .slice(0, 40)}`;
  ...
  const handleShare = async () => {
    ...
    setBusy(true);
    try {
      await nativeShareProductCard(
        { node: cardRef.current, filename },
        `${product.name} — on ZANGO`,
      );
    } catch (error: any) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="flex justify-center overflow-hidden rounded-3xl bg-[#2A1B16] p-4">
          <div
            style={{
              width: PRODUCT_SHARE_CARD_WIDTH * scale,
              height: PRODUCT_SHARE_CARD_WIDTH * scale,
            }}
          >
        ...
        <p className="text-center text-xs text-muted-foreground">
          Instagram-ready square · {PRODUCT_SHARE_CARD_WIDTH}×{PRODUCT_SHARE_CARD_WIDTH}px
        </p>
          ...
          {canShare && (
            ...
            >
              {busy ? (
                ...
                <>
                  <Share2 className="mr-2 h-4 w-4" /> Share to Instagram, WhatsApp…
                </>
              )}
            </Button>
          )}
