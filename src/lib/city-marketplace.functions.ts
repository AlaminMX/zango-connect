...
export async function getCityMarketplace({ data }: { data: { slug: string } }) {
  ...
  const [
    ...
    supabase
      .from("sellers")
      .select(
        "id, business_name, slug, category, city, profile_photo_url, cover_photo_url, is_verified, rating, bio, whatsapp_number",
      )
      .eq("city_id", city.id)
      .eq("status", "active")
    ...
    supabase
      .from("products")
      .select(
        "id, name, price, image_url, stock_status, status, seller_id, is_featured, featured_order, sellers!inner(id, business_name, slug, city_id, status, verification_status, is_blocked, whatsapp_number, is_verified)",
      )
  ...
  return {
    city: {
      ...
      name: city.name,
      slug: city.slug,
      state: city.name.toLowerCase() === "zaria" ? "Zaria" : (city.states?.name ?? ""),
      state_slug: city.name.toLowerCase() === "zaria" ? "zaria" : (city.states?.slug ?? ""),
    },
    sellers: sellers ?? [],
    products: products ?? [],
...
export async function getCityBySlug(slug: string) {
  const { data } = await supabase.from("cities_of_business").select("*").eq("slug", slug).single();
  return data;
}
